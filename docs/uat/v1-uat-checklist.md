...
- **Scenarios — all must pass:**
  ...
  3. [ ] Scan known beauty product (fallback chain → beauty-facts host) `[automated by SR-82 — OffApiFallbackChainTest.scenario2_foodMiss_beautyHit_returnsBeautyResult]`
  4. [ ] Scan known pet food (fallback chain → pet-food host) `[automated by SR-82 — OffApiFallbackChainTest.scenario3_foodBeautyMiss_petFoodHit_returnsPetFoodResult]`
  5. [ ] Scan known generic product (fallback chain → products-facts host) `[automated by SR-82 — OffApiFallbackChainTest.scenario4_foodBeautyPetFoodMiss_productsHit_returnsProductsResult]`
