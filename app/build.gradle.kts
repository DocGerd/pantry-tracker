...
android {
    ...
    buildTypes {
        getByName("debug") {
            ...
            // coverage from the JVM-only 18.74% to the combined 86.85%.
...
//
// PHASE 2 (this change, SR-180) merges instrumented (connectedDebugAndroidTest,
// emulator-gated per SR-79) execution data into the SAME report via the debug
// build type's `enableAndroidTestCoverage = true`, and raises the gate to 0.80.
// The measured COMBINED baseline (JVM unit/Robolectric + the 39 instrumented
// emulator tests) is 86.85% instruction / 88.78% line — comfortably clearing
// the 0.80 MUST, so the #140 silver test_statement_coverage80 row flips
// Unmet -> Met on the demonstrated coverage.
//
// The gate runs in the emulator CI job (.github/workflows/ci.yml `androidTest`)
// as a NON-required signal: it is deliberately NOT added to the required status
// checks and NO ruleset is edited, so a coverage regression shows red there
// without coupling merges to emulator reliability. The OpenSSF MUST is satisfied
// by the demonstrated >=80%, not by a merge-blocking check.
//
