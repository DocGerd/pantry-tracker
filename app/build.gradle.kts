...
//
// The local :app:jacocoTestCoverageVerification (run in the emulator CI job
// .github/workflows/ci.yml `androidTest`) is an informational mirror — not
// itself a required check, to avoid coupling merges to emulator reliability.
// The enforced, merge-blocking gate is the `codecov/project` >=80% line-coverage
// status check (codecov.yml), added as a required check on the develop + main
// rulesets once it first reports green (#219).
//
    ...
    violationRules {
        rule {
            limit {
                ...
                // line, so 0.80 clears comfortably. The intended merge-blocking
                // gate is the `codecov/project` >=80% check (#219; added as a
                // required ruleset check once it first reports green), which also
                // scores line coverage from this report's XML; this local task is
                // a dev convenience. Never run :app:jacocoTestCoverageVerification
