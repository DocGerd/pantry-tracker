...
android {
    ...
    buildTypes {
        getByName("debug") {
            // SR-180: emit JaCoCo execution data from instrumented
            // (connectedDebugAndroidTest) runs. AGP writes the coverage `.ec`
            // under build/outputs/code_coverage/debugAndroidTest/connected/,
            // which jacocoTestReport's executionData fileTree (rooted at
            // buildDirectory, globbing **/*.ec) picks up automatically — so the
            // emulator's instrumented coverage merges with the JVM unit-test
            // .exec into one combined report. This is what raises measured
            // coverage from the JVM-only ~19% to the combined ~85.93% line
            // (~81.37% instruction).
...
// SR-169 / SR-180 — JaCoCo statement-coverage reporting + gate (OpenSSF Silver
// `test_statement_coverage80` MUST, #140).
//
// PHASE 1 (#179) measured JVM unit-test coverage only (testDebugUnitTest) with
// the verification gate at 0.00 so it could not fail — a measured baseline
// recorded in the CI artifact while the test suite was grown.
//
// PHASE 2 (SR-180) merges instrumented (connectedDebugAndroidTest,
// emulator-gated per SR-79) execution data into the SAME report via the debug
// build type's `enableAndroidTestCoverage = true`, and gates at 0.80 LINE
// (statement) coverage (#219). The measured COMBINED baseline (JVM unit/
// Robolectric + the instrumented emulator suite) is ~85.93% line / ~81.37%
// instruction — comfortably clearing the 0.80 MUST, so the #140 silver
// test_statement_coverage80 row is Met on the demonstrated coverage.
//
// The active, merge-blocking coverage gate is the local
// :app:jacocoTestCoverageVerification, run in the emulator CI job
// .github/workflows/ci.yml `androidTest` — whose `androidTest` status check is
// REQUIRED on the develop ruleset (16993554). A `codecov/project` >=80%
// line-coverage status check is also configured (codecov.yml) but does not
// currently post (account-level Codecov issue — #228), so it is NOT the
// required gate; the main ruleset (16948699) coverage promotion is deferred.
//
    ...
    violationRules {
        rule {
            limit {
                ...
                // line, so 0.80 clears comfortably. This task IS the active
                // merge-blocking gate — it runs in the emulator `androidTest` CI
                // job, whose status check is required on the develop ruleset
                // (#219, #228). A `codecov/project` >=80% check (codecov.yml) was
                // intended but never posts (account-level — #228), so it is not
                // the gate. Never run :app:jacocoTestCoverageVerification
