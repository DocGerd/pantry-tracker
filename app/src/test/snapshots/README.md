...
- **Goldens are host-deterministic, not host-portable.** The RNG/Skia pixel
  output is pinned by the Robolectric version, but it still differs subtly
  between environments (verified: regenerating on a developer box produced PNGs
  that mismatched the CI-committed goldens for tests whose content had not
  changed). Regenerate goldens **in the CI environment** — delete the stale
  PNG(s), push, let the screenshot job's first-run write the fresh golden (it
  fails once), download/commit it, and re-run to green. Do not commit goldens
  rendered on a non-CI host.
- The brand-identity change (refined launcher icon + full M3 light/dark scheme)
  intentionally deletes `icon_full_canvas.png`, `icon_circular_mask.png`,
  `icon_square_mask.png`, `theme_light_mode.png` and `theme_dark_mode.png`;
  these are regenerated by CI per the procedure above.
