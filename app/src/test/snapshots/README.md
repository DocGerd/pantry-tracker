...
- **Goldens are host-deterministic, not host-portable.** The RNG/Skia pixel
  output is pinned by the Robolectric version, but still differs subtly between
  environments (verified: this project's dev box renders different bytes than CI
  even for screens whose content has not changed). Regenerate goldens **in an
  environment that matches CI** and commit those bytes — do not commit goldens
  rendered on a mismatched host.
  > ⚠️ There is currently **no** CI step that uploads the rendered goldens as an
  > artifact, so they cannot be retrieved from a failed CI run. See issue #237
  > for the maintenance plan (add an artifact/record path, or regenerate in a
  > CI-matching container).
- The DocGerdSoft brand change (refined launcher icon + full M3 light/dark
  scheme) invalidated `icon_full_canvas.png`, `icon_circular_mask.png`,
  `icon_square_mask.png`, `theme_light_mode.png`, and `theme_dark_mode.png`.
  These were deleted and **`AppIconScreenshotTest` + `ThemeScreenshotTest` are
  temporarily `@Ignore`d** (see #237) until the goldens are regenerated in a
  CI-matching environment and the tests re-enabled.
