...
- Monochrome / themed-icon layer (Android 13+) on the adaptive launcher icon, so
  the OS can tint the jars-and-shelf foreground to the user's monochrome theme.
- README hero banner (`docs/brand/hero.svg`) at the top of the README; the crisp
  2560×1120 PNG export is deferred to the maintainer.
...
- Expanded the Material 3 theme from the previous primary-only override into a
  full hand-built light/dark color scheme seeded from Fern `#4F7942` (every M3
