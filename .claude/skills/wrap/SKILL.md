...
2. Fold any durable lessons into the **project** `CLAUDE.md` at the repo root (not `~/.claude/CLAUDE.md`). Prefer the `claude-md-management:revise-claude-md` skill, or edit `CLAUDE.md` directly if that skill isn't loaded.
...
4. Confirm clean working tree via `git status`. If dirty, list the uncommitted/untracked files, halt, and tell the user; re-invoke wrap after they've resolved it.
